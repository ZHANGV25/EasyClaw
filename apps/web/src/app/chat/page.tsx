...
export default function ChatPage() {
  ...
  // In a real app, this would sync with server state
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-[var(--color-border-subtle)] bg-[var(--color-bg-primary)]/60 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="hidden sm:block px-3 py-1 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border-subtle)] text-xs text-[var(--color-text-secondary)] hover:border-[var(--color-accent)] hover:text-[var(--color-text-primary)] transition-colors cursor-pointer"
            >
              ${credits.toFixed(2)}
            </button>
            {/* Mobile credits */}
            <button 
              onClick={() => setShowAddCreditsModal(true)}
              className="sm:hidden px-2 py-1 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border-subtle)] text-xs text-[var(--color-text-secondary)] hover:border-[var(--color-accent)] hover:text-[var(--color-text-primary)] transition-colors cursor-pointer"
            >
              ${credits.toFixed(2)}
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 custom-scrollbar">
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            {/* Typing indicator — shown before first token */}
        ...
        {/* Input */}
        <div className="border-t border-[var(--color-border-subtle)] bg-[var(--color-bg-primary)]/80 backdrop-blur-md px-4 sm:px-6 py-4">
          <form
